...
\begin{center}
	\begin{figure}[ht!]
		\begin{tikzpicture}[level distance=3cm, sibling distance=3cm,every node/.style={rectangle, draw=black}]

		    \node{Gerente Técnico} [anchor=north]
		    [edge from parent fork down]
		    child{node[anchor=east] {Unidad SQA}}
		    child{node[anchor=south] {Jefe de proyecto}
		      child {node[anchor=east] {Desarrollo Front-End}}
		      child {node[anchor=south]{Desarrollo Back-End}}
		      child {node[anchor=west] {Unidad de soporte y mantenimiento}}}
		    child {node[anchor=west] {Unidad SCM}};
		\end{tikzpicture}
